...
const appConfig = window.ANKETA_CONFIG || {};
const API_BASE_URL = String(appConfig.API_BASE_URL ?? "http://127.0.0.1:8788").replace(/\/$/, "");
const CRM_PROXY_PORT = appConfig.CRM_PROXY_PORT || 8788;
const CRM_PROXY_HEALTH_ENDPOINT = `${API_BASE_URL}${appConfig.HEALTH_ENDPOINT || "/health"}`;
const CRM_IMPORT_ENDPOINT = `${API_BASE_URL}${appConfig.CRM_IMPORT_ENDPOINT || "/api/crm-import"}`;
...
async function checkCrmProxyHealth() {
  try {
    ...
    if (data.service && data.service !== "anketa-crm-proxy") {
      const error = new Error("CRM_PROXY_WRONG_SERVICE");
      error.crm = data;
      throw error;
    }
    return true;
  } catch (error) {
    if (error?.message === "CRM_PROXY_WRONG_SERVICE") throw error;
    return false;
  }
}
...
async function fetchCrmOrderViaProxy(rawLink) {
  ...
  try {
    response = await fetch(CRM_IMPORT_ENDPOINT, {
      method: "POST",
      cache: "no-store",
      headers: {
...
function getCrmErrorMessage(error) {
  ...
  if (error?.message === "CRM_PROXY_UNAVAILABLE") {
    if (!isLocalMode) {
      return "CRM-backend недоступний. Перевірте production API URL у config.js або стан backend-сервісу.";
    }
    ...
    const fileHint = window.location.protocol === "file:"
      ...
      : "";
    return `CRM-проксі анкети недоступний. Запустіть start-crm-proxy.cmd. Перевірте, чи запущений сервер на ${CRM_PROXY_PORT}.${fileHint}`;
  }

  if (error?.message === "CRM_PROXY_WRONG_SERVICE") {
    return `CRM-проксі анкети не відповідає. Перевірте, чи запущений сервер на ${CRM_PROXY_PORT}.`;
  }
  ...
  if (crm?.code) {
    ...
    if (crm.code === "domain_not_allowed") {
      return "Неправильне або непідтримуване CRM-посилання.";
    }
    if (crm.code === "short_link_resolve_failed") {
      return "CRM-сторінка недоступна або не відповідає.";
    }
    if (crm.code === "api_url_not_found") {
      return "CRM-проксі працює, але не вдалося розпізнати дані замовлення. Можливо, змінилася структура CRM.";
    }
